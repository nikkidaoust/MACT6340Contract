...
async function main() {
  ...
  const NikkiNFTContractFactory = await ethers.getContractFactory(
    "NikkiNFTContract"
  );
  // deploy
  const NikkiNFTContract = await NikkiNFTContractFactory.deploy(
    args.mint_price,
    args.max_tokens,
    args.base_uri,
    args.royaltyArtist,
  ...
  await NikkiNFTContract.deploymentTransaction().wait(15);
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await NikkiNFTContract.mintTo(ipfs[i], {
