...
error NikkiNFTContract__MaxSupplyReached();
error NikkiNFTContract__ValueNotEqualPrice();
error NikkiNFTContract__WrongAvenueForThisTransaction();

/// @custom:security-contact ndaoust@smu.edu
contract NikkiNFTContract is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, ERC721Royalty, Ownable, ReentrancyGuard {
    ...
    receive() external payable {
        revert NikkiNFTContract__WrongAvenueForThisTransaction();
    }
    ...
    function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert NikkiNFTContract__MaxSupplyReached();
        }
        // make sure there us the right amount of money
        if (msg.value != i_mint_price) {
            revert NikkiNFTContract__ValueNotEqualPrice();
        }
        _tokenIdCounter++;
